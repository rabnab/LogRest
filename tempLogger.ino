...
void setup()
{
    ...
    status = comm->initializeWiFi(status);
...
void loop()
{
    ...
    if (currentMillisInfo - previousMillisInfo >= intervalInfo)
    {
        ...
        if (senH->updateSensorValues()) {
            char *serialOutput=comm->postValuesToServer(
    ...
    if ((currentMillisInfo - previousMillisReconnect) > reconnectInterval)
    {
        ...
        int wifiState = comm->getWifiState();
        if (wifiState != WL_CONNECTED)
        {
            print("wifi state: ");
            println(comm->translateWifiState(wifiState));
            println("resetting wifi due to connection loss.");
            status = comm->initializeWiFi(wifiState);
        }
